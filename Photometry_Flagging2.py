...
in_name= os.path.join(path, 'YBphotometry_results_' + instIDin + '.csv')
...
reviewcsv= ascii.read(in_name, delimiter=',')
...
while (YB1 < YB2) and not done:
    ...
    reviewflag = list(reviewcsv[YB1 + 1])[-11:]
    YB= data[YB1]['YB']
    YB_long= data[YB1]['l']
    YB_lat= data[YB1]['b']
    YB_rad= data[YB1]['r']
    ...
    if int(reviewflag[-1])==1:
        flag= make_flags(origlist, avmasks, resids, interps, heatmaps, wcslist, YB)
    else:
        satflags = []
        for i in range(len(avmasks)):
            if np.max(avmasks[-i-1]) != 0:
                satflags.append(0)
            else:
                satflags.append(1)
        flag= satflags + reviewflag
    #####################################################
    # Writing to CSV                                    #        
    #####################################################
